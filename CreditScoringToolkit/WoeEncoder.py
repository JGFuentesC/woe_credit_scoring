...
class WoeEncoder:
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, target_col: str = 'binary_target') -> None:
        ...
        aux[target_col] = y
        self._woe_encoding_map = dict(ChainMap(*map(lambda feature: self._woe_transformation(aux, feature, target_col), self.features)))
    ...
    def _woe_transformation(X: pd.DataFrame, feature: str, bin_target: str) -> Dict[str, Dict]:
        ...
        if X[bin_target].nunique() != 2:
            raise ValueError(f"The target column '{bin_target}' must have exactly 2 unique values.")
    ...
    def inverse_transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        aux = X.copy()
        self._woe_reverse_map = {feature: {v: k for k, v in woe_map.items()} for feature, woe_map in self._woe_encoding_map.items()}
